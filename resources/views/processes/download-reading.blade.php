...
<body id="page-top">
    ...
    <script>
        ...
        function loadBillMonths(readerId) {
            fetch(`{{ route('meter-reading.assignments') }}?reader_id=${readerId}&get_bill_months=1`)
                .then(response => response.json())
                .then(data => {
        ...
        function loadRoutesForBillMonth(readerId, billMonth) {
            ...
            // Fetch routes for specific bill month
            fetch(`{{ route('meter-reading.assignments') }}?reader_id=${readerId}&bill_month=${billMonth}`)
                .then(response => response.json())
                .then(data => {
                    if (data.success && data.data.length > 0) {
                        currentModalRoutes = data.data;
                        ...
                        modalRefreshInterval = setInterval(function() {
                            fetch(`{{ route('meter-reading.assignments') }}?reader_id=${currentModalReaderId}&bill_month=${currentBillMonth}`)
                                .then(r => r.json())
                                .then(d => {
                                    if (d.success && d.data) {
                                        currentModalRoutes = d.data;
                                        applyRouteSearch();
                                    }
                                }).catch(() => {});
                        }, 45000);
